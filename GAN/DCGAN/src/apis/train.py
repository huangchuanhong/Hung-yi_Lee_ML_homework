...
def train(args):
    ...
    dataset = FacesDataset(args.data_root)
    ...
    discriminator = DCGANDiscriminator(3, args.dim)
    if args.discriminator_load_from:
        load_checkpoint(discriminator, args.discriminator_load_from)
    discriminator = discriminator.to(args.device)
    g_optimizer = torch.optim.Adam(generator.parameters(), lr=args.lr, betas=(0.9, 0.999))
    d_optimizer = torch.optim.Adam(discriminator.parameters(), lr=args.lr, betas=(0.9, 0.999))
    ...
    for epoch in range(args.total_epoch):
        ...
        while((i * args.discriminator_iters) < len(dataloader) - args.discriminator_iters):
            ...
            while train_discriminator < args.discriminator_iters:
                ...
                data = next(dataloader).to(args.device)
